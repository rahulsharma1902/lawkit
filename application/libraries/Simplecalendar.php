...
class Simplecalendar{
	 ...
	 * @return string
	 */
	public function render($effect = null, $startPeriod = null, $excludeweekends = null, $firstDayPeriod = NULL, $endDayPeriod = NULL) {
		$festivedays = array(
			date("Y-01-01"),//Año nuevo
			date("Y-02-03"),//Const mexicana
			date("Y-03-16"),//Nat Benito J
			date("Y-05-01"),//Dia trab
			date("Y-09-16"),//Independencia
			date("Y-11-16"),//Rev mex
			date("Y-12-25"),//Navidad
		);
		$dayOfEffect = NULL;
		$isEndDay = "";
		$isFirstDay = "";
		$classexcludeweekends = "";
		$apply_color_fday = null;
		$apply_effect_color = 0;
		...
		$out = <<<TAG
<table class="{$this->classes['calendar']}"><thead><tr>
TAG;
...
TAG
		...
		for( $i = 1; $i <= $daysInMonth; $i++ ) {
			...
			if($effect == 1){
				$dayOfEffect = new DateTime(date('Y-m-d', strtotime($startPeriod . ' + 1 days')));
				$dayOfEffect = $dayOfEffect->format("Y-m-d");
			}

			if($excludeweekends != 2){
				if( is_array($dailyHTML) ) {
					$dayEffect  = ($dayOfEffect    == $date->format("Y-m-d")) ? "markAsEffect" : NULL;
					$isFirstDay = ($firstDayPeriod == $date->format("Y-m-d")) ? "isFirstDay" : NULL;
					$isEndDay   = ($endDayPeriod   == $date->format("Y-m-d")) ? "isEndDay" : NULL;
					$isFestive  = (in_array($date->format("Y-m-d"), $festivedays)) ? "isFestive" : NULL;
					$out .= '<td>';
					$out .= '<div class="' . $isFestive . ' ' . $dayEffect . ' ' . $isEndDay . ' ' . $isFirstDay . ' has-event ' . ($isToday ? $this->classes['today'] : NULL) . '">' . $i;
					...
					$isFestive  = (in_array($date->format("Y-m-d"), $festivedays)) ? "isFestive" : NULL;
					$out .= '<td>';
					$out .= '<div  class="' . $isFestive . ' ' . $dayEffect . ' ' . $isEndDay . ' ' . $isFirstDay . ' ' . ($isToday ? $this->classes['today'] : NULL)  . '">' . $i . '</div>';
				}
			}
			else{
				$classexcludeweekends = "SCprefix";
				if($date->format("N") < 6){
					if( is_array($dailyHTML) ) {
						$dayEffect  = ($dayOfEffect    == $date->format("Y-m-d")) ? "markAsEffect" : NULL;
						$isFirstDay = ($firstDayPeriod == $date->format("Y-m-d")) ? "isFirstDay" : NULL;
					...
					if( is_array($dailyHTML) ) {
						...
						$isFestive  = (in_array($date->format("Y-m-d"), $festivedays)) ? "isFestive" : NULL;
						$out .= '<td>';
						$out .= '<div class="' . $isFestive . ' ' . $dayEffect . ' ' . $isEndDay . ' ' . $isFirstDay . ' ' . $classexcludeweekends.' has-event ' . ($isToday ? $this->classes['today'] : NULL) . '">' . $i;
						foreach( $dailyHTML as $dHtml ) {
							$out .= $dHtml;
						}
						$out .= '</div>';
						...
						$isFestive  = (in_array($date->format("Y-m-d"), $festivedays)) ? "isFestive" : NULL;
						$out .= '<td>';
						$out .= '<div class="' . $isFestive . ' ' . $dayEffect . ' ' . $isEndDay . ' ' . $isFirstDay . ' ' . $classexcludeweekends . ' ' . ($isToday ? $this->classes['today'] : NULL)  . '">' . $i . '</div>';
					}
				}
			}
			$out .= '</td>';
